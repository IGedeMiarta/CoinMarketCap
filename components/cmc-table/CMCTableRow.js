...
const CMCTableRow = ({
  ...
  const getRandomGraph =()=>{
    ...
    const viewCoinDetails = () => {
        router.push(`/currencies/info?symbol=${coinSymbol}&coinName=${coinName}&price=${price}`);
    }
    const viewPrice = () => {
        router.push(`/currencies/price?symbol=${coinSymbol}&coinName=${coinName}&price=${price}`);
    }
    const formatNum = num => {
  ...
  return (
    <div className={styles.tableRow}>
      <tr>
          ...
          <td>
                <Rate isIncrememt={hRateIsIncrement} rate={`${formatNum(hRate)}%`} />
          </td>
          <td>
              <Rate isIncrememt={dRateisIncrement} rate={`${formatNum(dRate)}%`} />
          </td>
          <td>
              <div>
